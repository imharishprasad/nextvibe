...
export default function Loader({
  ...
  const effects: Record<string, any> = {
    ...
    rotate: { initial: { rotate: -10 }, animate: { rotate: [0, 10, -10, 0] } },
  };

  const letterAnimations: Record<
    string,
    (index: number) => { initial: any; animate: any; transition: any }
  > = {
    "letter-assemble": (index) => ({
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div
          className={`fixed inset-0 flex items-center justify-center ${bgColor} ${textColor}`}
          initial={{ opacity: 1 }}
          exit={{ opacity: 0, transition: { duration: 0.5 } }}
        >
          {["letter-assemble", "bubble", "wave"].includes(effect) ? (
            <motion.div className="flex space-x-2 text-6xl font-bold">
              {text.split("").map((char, index) => (
                <motion.span key={index} {...letterAnimations[effect]?.(index)}>
                  {char}
                </motion.span>
              ))}
            </motion.div>
